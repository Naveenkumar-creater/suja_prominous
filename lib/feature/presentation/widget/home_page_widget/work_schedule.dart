...
class BarChartExample extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final  textColor = themeState.isDarkTheme ?   Colors.white: const Color(0xFF424242) ;
    ...
    List<charts.Series<ChartData, String>> series = [
      charts.Series(
        ...
        colorFn: (ChartData data, _) => charts.ColorUtil.fromDartColor(categoryColors[data.category] ?? Colors.blue),
      ),
    ];

    return Scaffold(
      backgroundColor: themeState.isDarkTheme ? const Color(0xFF424242) : Colors.white,
      body: Padding(
          ...
          domainAxis: charts.OrdinalAxisSpec(
            renderSpec: charts.SmallTickRendererSpec(
              labelStyle: charts.TextStyleSpec(
                color: charts.ColorUtil.fromDartColor(textColor), // Use the determined text color here
              ),
            ),
          ),
        ),
