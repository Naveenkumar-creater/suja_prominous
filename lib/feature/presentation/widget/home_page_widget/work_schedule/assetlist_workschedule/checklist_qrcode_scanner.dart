...
class QRCodeScannerPage extends StatefulWidget {
  ...
  const QRCodeScannerPage(this.expectedBarcode, this.plantId, this.acrpinspectionstatus, this.assetId);
      ...
      if (barcode == widget.expectedBarcode) {
        setState(() {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => CaptureImage(context, widget.plantId,assetIdPage,widget.acrpinspectionstatus,widget.assetId),
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            ...
            if (showScanButton)
              Card(elevation: 5,shadowColor: Colors.black,
                child: IconButton(iconSize: 50,
                  onPressed: _scanBarcode, icon:   const Icon(Icons.qr_code,color: Colors.blue,),
                 
                ),
              ),const SizedBox(height: defaultPadding/2,),const Text('Scan Again' ,style: TextStyle(fontSize: 20.0)),
          ],
        ),
      ),
    );
